...
data("BostonHousing") #라이브러리안에 있는 여러 데이터 셋중 한개를 사용하기
...
#문자형 자료를 범주형 자료(factor) 형변환, 알파벳 순서대로 만들어짐
grp <- factor(grp)
class(grp)
head( grp )
factor( grp, levels=c('H','M','L')) #factor의 순서를 사용자가 정의하기
head(grp)

head( myds )
grp <- data.frame(myds, grp)
head( myds )
...
# 그룹별 관측값 분포 확인
par(mfrow=c(1,1))
# 구할려는 값(y축) - 그룹화할변수(x축), data =데이터셋
boxplot(myds$crim~myds$grp, data = myds, main = '1인당 범죄율')

boxplot(myds$rm~myds$grp, data = myds, main = '방의 갯수에 다른 분류')
