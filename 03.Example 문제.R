...
#2단계 matrix -> data.frame
st <- data.frame( state.x77)
class(st)
#3단계 조건으로 검출
rics_state.csv <- subset( st, st$Income >= 5000)[c('Income', 'Population', 'Area')]
...
#풀이
#4단계 저장하기
write.csv(rich_state, ('d:/a1/study/R_study/rich_state.csv'))
ds <- read.csv(('d:/a1/study/R_study/rich_state.csv'))
...
#3단계
rownames( subst(mtcars, mtcars$mpg == max( mtcars$mpg)))
