...
class TrafficWeak(QMainWindow) :
    def __init__(self) :
        ...
        signButton = QPushButton('표지판등록', self)
        roadButton = QPushButton('도로영상불러옴', self)
    ...
    def signFunction(self):
        self.label.clear()
        self.label.setText('교통 약자표지판을 등록합니다')
        ...
        if self.roadImg is None:
           self.label.setText('먼저 도로 영상을 입력하세요')
            ...
            if len(GM[best]) < 4:
                ...
                h1.w1 = self.signImg[best].shape[0], self.signImgs[best].shape[1]
                h2.w2 = self.roadImg[best].shape[0], self.signImg.shape[1]
